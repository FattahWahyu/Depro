    ...
    if (confirmDelete) {
      ...
      if (token) {
        ...
        const response = await axios.put(
          `https://c23-gt01-01.et.r.appspot.com/products/${product.id}`,
          updatedProductData,
          config
        );
        ...
        <>
          <>
            {edited && data.length > 0 && (
              <div className="flex justify-end">
                <div
                  className="flex items-center bg-[#05C6FB] font-inter text-white font-bold w-fit px-2 rounded-2xl text-xl hover:bg-[#71533a] hover:animate-pulse drop-shadow-2xl"
                  onClick={deleteProcessHandle}
                >
                  <Icon nonactive>
                    <MdClear />
                  </Icon>
          ...
          <div className="flex mt-4">
            <div className="md:w-2 w-1 h-[1] box-border md:pt-32 md:pb-24 translate-x-[50px] pt-32  md:translate-x-[100px] ">
              <div className="w-full h-full bg-[#05C6FB]"></div>
            </div>
            <div>
              {data.length > 0
